...
$totalExpeditions = count($transits);
$scheduledCount   = 0;
$activeCount      = 0;
$deliveredCount   = 0;
...
<head>
    ...
    <link rel="stylesheet" href="/transit/transit/public/css/style.css?v=2.9">
</head>
<body>
...
<div class="app-container">
    ...
    <main class="main-content">
        ...
        <nav class="top-navbar">
            <div class="navbar-title"><span>Expéditions</span></div>
        ...
        <header class="dashboard-header">
            <div class="header-title" data-aos="fade-right">
                <h1>Registre des Expéditions</h1>
                <p>Suivi détaillé de tous les transits de marchandises programmés en Guinée et à l'international</p>
            </div>
        </header>

        <!-- Grille de Statistiques Logistiques (Harmonisation UI) -->
        <div class="dashboard-grid" style="margin-bottom: 2rem;">
            ...
            <div class="dashboard-card card-blue-royal" data-aos="fade-up" data-aos-delay="0">
                <div class="card-header">
                    ...
                    <span class="card-trend badge-blue">Global</span>
                </div>
                <div class="card-body">
                    <h3 class="card-title">Total Expéditions</h3>
                    <p class="card-value"><?= $totalExpeditions ?></p>
                </div>
                <div class="card-footer">
                    <span class="card-meta">Transits enregistrés</span>
                </div>
            </div>
            ...
            <div class="dashboard-card card-blue-slate" data-aos="fade-up" data-aos-delay="100">
                <div class="card-header">
                    ...
                    <span class="card-trend badge-blue">Planifié</span>
                </div>
                <div class="card-body">
                    <h3 class="card-title">Programmées</h3>
                    <p class="card-value"><?= $scheduledCount ?></p>
                </div>
                <div class="card-footer">
                    <span class="card-meta">Départs à venir</span>
                </div>
            </div>
            ...
            <div class="dashboard-card card-blue-cyan" data-aos="fade-up" data-aos-delay="200">
                <div class="card-header">
                    ...
                    <span class="card-trend badge-blue">Transit</span>
                </div>
                <div class="card-body">
                ...
                <div class="card-footer">
                    <span class="card-meta">Sur les itinéraires</span>
                </div>
            </div>
            ...
            <div class="dashboard-card card-blue-indigo" data-aos="fade-up" data-aos-delay="300">
                <div class="card-header">
                    ...
                    <span class="card-trend badge-blue">Terminé</span>
                </div>
                <div class="card-body">
                ...
                <div class="card-footer">
                    <span class="card-meta">Arrivées validées</span>
                </div>
            </div>
        </div>

        <!-- Listing Section -->
        <section class="data-section" data-aos="fade-up">
            <div class="section-header">
                <h2>Tous les Transits</h2>
                <span class="badge badge-blue"><?= count($transits) ?> Transits Globaux</span>
            </div>
                ...
                <table class="custom-table">
                    <thead>
                        <tr>
                            ...
                            <th>Trajet Géographique</th>
                            <th>Date de Départ</th>
                            <th>Date d'Arrivée</th>
                            <th>Statut Transit</th>
                            <th>Moyen de Routage</th>
                            <th>Base Logistique</th>
                    ...
                    <tbody>
                        <?php foreach ($transits as $transit): ?>
                            <?php $marchandise = $transit->getMarchandise(); ?>
                            <tr>
                                <td><strong>#TR-<?= str_pad((string)$transit->getId(), 4, '0', STR_PAD_LEFT) ?></strong></td>
                                <td>
                                    <strong><?= htmlspecialchars($marchandise->getDesignation()) ?></strong>
                                </td>
                                <td>
                                    <strong><?= htmlspecialchars($transit->getVilleDepart()->getNom()) ?></strong> 
                                    ➜ 
                                    <strong><?= htmlspecialchars($transit->getVilleArrivee()->getNom()) ?></strong> 
                                </td>
                                <td><?= $transit->getDateDepart()->format('d/m/Y H:i') ?></td>
                                <td><?= $transit->getDateArrivee()->format('d/m/Y H:i') ?></td>
                                <td>
                                    <span class="badge <?= $transit->getStatutClass() ?>"><?= $transit->getStatut() ?></span>
                                </td>
                                <td>
                                    <span class="badge badge-blue"><?= $transit->getModeTransport()->getType() ?></span>
                                </td>
                                <td>
                                    <strong>
                                        <?php if ($transit->getModeTransport()->getType() === 'Maritime'): ?>
                                            <?= number_format($marchandise->getSurface(), 1, ',', ' ') ?> m²
                                        <?php else: ?>
                                            <?= number_format($marchandise->getPoids(), 1, ',', ' ') ?> kg
                                        <?php endif; ?>
                                    </strong>
                                </td>
                                <td><?= number_format($transit->getTarifUnitaire(), 0, ',', ' ') ?> GNF</td>
    ...
    <div class="modal-box" style="max-width: 700px;">
        ...
        <div id="details-edit-mode" style="display: none; flex-direction: column; overflow: hidden; flex-grow: 1;">
            <form method="POST" action="index.php" class="modal-form">
                ...
                <input type="hidden" name="redirect_to" value="/transit/transit/expeditions">
...
<script>
    ...
    function triggerDelete() {
        if (activeTransitData && confirm("⚠️ Êtes-vous sûr de vouloir supprimer définitivement ce transit #" + String(activeTransitData.id).padStart(4, '0') + " et sa facture de la base de données ? Cette action est irréversible !")) {
            window.location.href = "/transit/transit/dashboard?action=supprimer_transit&id=" + activeTransitData.id + "&redirect_to=" + encodeURIComponent("/transit/transit/expeditions");
        }
    }
</script>
</body>
